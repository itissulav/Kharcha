...
import { getCategoryWithTransaction, getMonthlyExpenses, MonthlyExpense } from '@/db/transactions';
...
const StatisticsScreen = () => {
  ...
  const PIE_CHART_COLORS = [
    '#6366F1', '#818CF8', '#A5B4FC', '#3B82F6', '#60A5FA',
    '#93C5FD', '#8B5CF6', '#A78BFA', '#C4B5FD', '#E0E7FF'
  ];
  ...
  const fetchData = useCallback(async () => {
    ...
    const total = categoriesWithTransactions.reduce((acc, cat) => acc + cat.total, 0);
  ...
  return (
    <SafeAreaView className="flex-1 bg-gray-950">
      ...
      >
        <View className="p-5">
          {/* Header */}
          <Text className="text-3xl font-bold text-white mb-1 overflow-hidden">Statistics</Text>
          ...
          {/* Line Chart Section */}
          {/**/}
          <Text className="text-lg font-semibold text-gray-200 mb-5">Monthly Expenses</Text>
          <ScrollView className="bg-primary rounded-2xl p-5 mb-6 border border-gray-800" horizontal>
            {lineData.length > 1 ? (
              <View style={{ width: screenWidth - 40 }} className='overflow-x-hidden w-full'>
                <LineChart
                  data={lineData}
                  isAnimated
                  areaChart
                  ...
                  yAxisThickness={0}
                  xAxisThickness={0}
                  onPress={({ index }: ChartPressProps) => setSelectedLineIndex(index)}
          ...
          {/* Pie Chart Section */}
          <Text className="text-lg font-semibold text-white mb-1">Spending by Category</Text>
          <Text className="text-sm text-gray-400 mb-4">
            This pie chart shows where your money goes. The percentage is based on total expenses.
          </Text>
              ...
              <>
                <View className="items-center my-4">
                  <PieChart
                    ...
                    onPress={(item: any) => {
                      if (item && item.label) {
                        setSelectedPieCategory((prev) => (prev === item.label ? null : item.label));
                      }
                    }}
                    radius={120}
                    innerRadius={70}
                      ...
                      <View key={item.label} className="flex-row items-center mb-3">
                        ...
                        <Text
                          className={`flex-1 text-base ${
                            selectedPieCategory === item.label ? 'text-white font-bold' : 'text-gray-300'
                          }`}
                        >
                          {item.label}
                        </Text>
                        <Text className="text-base font-semibold text-gray-50">
                          ${item.fullValue?.toFixed(2) ?? '0.00'}
                        </Text>
                        {selectedPieCategory === item.label && (
                          <Text className="text-sm text-indigo-400 ml-2">← Selected</Text>
                        )}
                      </View>
                    );
                  })}
